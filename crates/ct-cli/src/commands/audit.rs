...
use ct_security::{AuditFilter, AuditLog};
...
fn format_event(event: &ct_security::AuditEvent) -> String {
...
fn get_audit_log_path() -> anyhow::Result<std::path::PathBuf> {
    ...
    Ok(home.join(".citros").join("audit.log"))
}

#[cfg(test)]
mod tests {
    use super::*;
    use ct_security::{AuditEvent, AuditEventType};
