...
pub enum AuditEventType {
    ...
    // System events
    /// Citros agent started
    SystemStartup,

    /// Citros agent stopped
    SystemShutdown,
...
impl AuditEvent {
    ...
    fn current_timestamp_ms() -> Result<u64, ct_core::error::SecurityError> {
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map_err(|e| {
                ct_core::error::SecurityError::AuditLog(format!("Invalid system time: {}", e))
            })?
            .as_millis()
            .try_into()
            .map_err(|_| ct_core::error::SecurityError::AuditLog("Timestamp overflow".to_string()))
    }
    ...
    ) -> Result<Self, ct_core::error::SecurityError> {
