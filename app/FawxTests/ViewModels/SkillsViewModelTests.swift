...
final class SkillsViewModelTests: XCTestCase {
    ...
    func testSaveEditingSettingsSendsClearForSecretsAndClosesEditor() async throws {
        ...
        let appState = try await makeConfiguredAppState { request in
            ...
            XCTAssertEqual(request.url?.path, "/v1/skills/brave-search/settings")
            await capturedRequestBody.set(try request.bodyDataForTesting())
        ...
        let capturedBody = await capturedRequestBody.value()
    ...
    private func makeConfiguredAppState(
        ...
        await MockSkillsURLProtocol.setResponder(responder)
...
private actor CapturedRequestBody {
...
private final class MockSkillsURLProtocol: URLProtocol, @unchecked Sendable {
    ...
    static func setResponder(_ responder: @escaping MockSkillsURLProtocolStore.Responder) async {
...
private actor MockSkillsURLProtocolStore {
    typealias Responder = @Sendable (URLRequest) async throws -> MockSkillsResponse
