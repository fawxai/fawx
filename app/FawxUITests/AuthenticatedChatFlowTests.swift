...
final class AuthenticatedChatFlowTests: XCTestCase {
    ...
    func testAuthenticatedUserCanSendMessageInExistingSession() async throws {
        ...
        await enterComposerText(
            "Reply with exactly the single word FawxTest and nothing else.",
            into: messageInput,
            sendButton: sendButton,
            in: app
        )
        let didEnableSendButton = await waitForEnabled(sendButton, timeoutSeconds: 3)
        XCTAssertTrue(
            didEnableSendButton,
            "Expected the send button to become enabled after entering a message."
        )
        sendButton.tap()
    ...
    @MainActor
    private func focusComposerForTyping(_ messageInput: XCUIElement, in app: XCUIApplication) {
        if messageInput.isHittable {
            messageInput.tap()
            return
        }

        let frame = messageInput.frame
        XCTAssertFalse(frame.isEmpty, "Expected the message input to expose a non-empty frame.")

        // NSTextView inside NSScrollView can report a visible frame while XCTest
        // still fails element hit-point resolution. Fall back to a center tap.
    ...
    @MainActor
    private func enterComposerText(
        _ text: String,
        into messageInput: XCUIElement,
        sendButton: XCUIElement,
        in app: XCUIApplication
    ) async {
        focusComposerForTyping(messageInput, in: app)
        messageInput.typeText(text)

        if await waitForEnabled(sendButton, timeoutSeconds: 1.5) {
            return
        }
