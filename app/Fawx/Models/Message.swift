...
enum SessionContentBlock: Codable, Sendable, Hashable {
    ...
    var displayText: String? {
        ...
        case .toolUse(_, let name, let input):
            return toolUseSummary(name: name, input: input)
        case .toolResult:
            return nil
    ...
    private func toolUseSummary(name: String, input: JSONValue) -> String {
        let target = input.toolSummaryTarget
        if target.isEmpty {
            return "[\(name)]"
        }
        return "[\(name)] \(target)"
...
private extension JSONValue {
    ...
    var toolSummaryTarget: String {
        ...
        for path in preferredPaths {
            if let value = value(at: path)?.chatDisplayText, !value.isEmpty {
                return value
            }
        }
