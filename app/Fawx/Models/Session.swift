...
enum SessionArchiveFilter: String, Codable, CaseIterable, Sendable, Hashable {
    case active
    case all
    case archivedOnly = "only"
}
...
struct Session: Codable, Identifiable, Sendable, Hashable {
    ...
    init(from decoder: Decoder) throws {
        ...
        thinking = try container.decodeIfPresent(ThinkingLevel.self, forKey: .thinking)
        createdAt = try container.decode(Int.self, forKey: .createdAt)
        updatedAt = try container.decode(Int.self, forKey: .updatedAt)
        messageCount = try container.decode(Int.self, forKey: .messageCount)
        archived = try container.decodeIfPresent(Bool.self, forKey: .archived) ?? false
        archivedAt = try container.decodeIfPresent(Int.self, forKey: .archivedAt)
    }
}
...
struct StopSessionResponse: Codable, Sendable, Hashable {
    let key: String
    let stopped: Bool
}

struct SteerSessionResponse: Codable, Sendable, Hashable {
    let key: String
    let steered: Bool
    let reason: String?
}
...
extension Session {
    ...
    init(threadSummary: ThreadSummary) {
        self.init(
            key: threadSummary.activeSessionID,
            kind: SessionKind(threadKind: threadSummary.kind),
            status: SessionStatus(threadStatus: threadSummary.status),
            label: nil,
            title: threadSummary.displayTitle,
...
extension SessionStatus {
    init(threadStatus: ThreadStatus) {
        switch threadStatus {
        case .active:
            self = .active
        case .idle:
