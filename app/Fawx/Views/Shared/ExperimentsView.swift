...
struct ExperimentsView: View {
        ...
        experimentsContent
            .overlay {
                GeometryReader { proxy in
                    selectedExperimentPanel(availableHeight: proxy.size.height)
...
#if os(macOS)
    @ViewBuilder
    private func selectedExperimentPanel(availableHeight: CGFloat) -> some View {
        if viewModel.selectedExperimentID != nil {
            ExperimentDetailFloatingPanel(viewModel: viewModel)
                .frame(width: 520)
                .frame(maxHeight: experimentPanelMaxHeight(availableHeight: availableHeight))
    ...
    private func experimentPanelMaxHeight(availableHeight: CGFloat) -> CGFloat {
        let availablePanelHeight = availableHeight - (FawxSpacing.paddingLG * 2)
        return min(640, max(240, availablePanelHeight))
    }
#endif
...
private struct ExperimentDetailSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ExperimentDetailContent(viewModel: viewModel)
            .background(Color.fawxBackground)
...
#if os(macOS)
private struct ExperimentDetailFloatingPanel: View {
    @Bindable var viewModel: ExperimentsViewModel
        ...
        VStack(spacing: 0) {
            HStack(alignment: .center, spacing: FawxSpacing.paddingSM) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(viewModel.selectedExperiment?.name ?? "Experiment")
                ...
                .buttonStyle(.plain)
                .accessibilityLabel("Close experiment details")
            }
            ...
            ExperimentDetailContent(viewModel: viewModel, contentPadding: FawxSpacing.paddingMD)
        }
...
private struct ExperimentDetailContent: View {
    @Bindable var viewModel: ExperimentsViewModel
    let contentPadding: CGFloat

    init(viewModel: ExperimentsViewModel, contentPadding: CGFloat = FawxSpacing.paddingLG) {
        _viewModel = Bindable(viewModel)
