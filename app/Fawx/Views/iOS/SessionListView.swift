...
struct SessionListView: View {
  ...
  @ViewBuilder
  private var workspacePickerSection: some View {
    if sessionViewModel.workspaces.count > 1 {
      Section("Workspace") {
        Picker(
          "Workspace",
          selection: Binding(
            get: {
              sessionViewModel.selectedWorkspaceID ?? sessionViewModel.workspaces.first?.id ?? ""
            },
            set: { selectWorkspace($0) }
          )
        ) {
          ForEach(sessionViewModel.workspaces) { workspace in
    ...
    if let backgroundActivityNotice {
      Section("Activity") {
        VStack(alignment: .leading, spacing: FawxSpacing.paddingXS) {
          Text(backgroundActivityUsesOverviewMessage ? backgroundActivityNotice.overviewMessage : backgroundActivityNotice.message)
            .font(FawxTypography.sidebarTitle)
            .foregroundStyle(Color.fawxText)

          Text(backgroundActivityNotice.detail)
    ...
    case .newSession:
      mobileChatShell(
        ChatDetailView(
          ...
          emptyStateMessage: "Start typing and Fawx will create a new conversation."
        )
  ...
  private func selectWorkspace(_ workspaceID: String) {
    sessionViewModel.selectWorkspace(workspaceID)
    guard usesSplitLayout == false else {
      return
    }
  ...
  private func presentInitialRouteIfNeeded() {
    ...
    if let selectedSessionID = sessionViewModel.selectedSessionID {
      navigationPath = [.session(selectedSessionID)]
    } else {
      chatViewModel.cancelScheduledLoad()
      sessionViewModel.select(nil)
      chatViewModel.showEmptyState()
  ...
  private func sessionRow(for session: Session) -> some View {
    let activity = sessionViewModel.activitySnapshot(for: session)
    let rowContent = SessionRowView(
      session: session,
      isSelected: session.id == sessionViewModel.selectedSessionID,
      isStreaming: activity?.isRunning ?? chatViewModel.activeStreamSessionIDs.contains(session.id),
      activityLabel: nil,
      showsUnreadDot: activity?.showsUnreadIndicator ?? false
    )
  ...
  private var backgroundActivityNotice: BackgroundThreadActivityNotice? {
    if usesSplitLayout {
      if let selectedNotice = sessionViewModel.selectedBackgroundActivityNotice {
        return selectedNotice
      }

      guard sessionViewModel.selectedThreadID == nil else {
        return nil
      }

      return sessionViewModel.backgroundActivityOverviewNotice
    }

    return sessionViewModel.backgroundActivityOverviewNotice
  }

  private var backgroundActivityUsesOverviewMessage: Bool {
    !usesSplitLayout || sessionViewModel.selectedThreadID == nil
  }
