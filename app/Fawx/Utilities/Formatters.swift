...
func compactRelativeTimestampString(_ epochSeconds: Int) -> String {
    let now = Int(Date().timeIntervalSince1970)
    let delta = max(now - epochSeconds, 0)

    if delta < 60 {
        return "\(max(delta, 1))s"
    }
    if delta < 3_600 {
        return "\(max(delta / 60, 1))m"
    }
    if delta < 86_400 {
        return "\(max(delta / 3_600, 1))h"
    }
    return "\(max(delta / 86_400, 1))d"
}
...
func modelMetadataSummary(_ model: ModelInfo) -> String {
    modelProviderMetadataSummary(model)
}

func modelProviderMetadataSummary(_ model: ModelInfo) -> String {
