...
struct FawxApp: App {
    ...
    var body: some Scene {
        ...
        let _ = FawxAccentPalette.update(appState.accentColor)
#if os(macOS)
    ...
    private func themedRootView(selectedTheme: AppTheme) -> some View {
        ...
        let fawxRoot = rootViewWithPermissionSheet
            .fawxAccentInvalidation(appState.accentColor)

        if #available(macOS 15.0, *) {
            fawxRoot
                .containerBackground(Color.fawxBackground, for: .window)
        } else {
            fawxRoot
        }
#else
    ...
    private func refreshForForegroundActivation() async {
        ...
        do {
            guard !chatViewModel.isStreaming else {
                // Appearance and foreground transitions can fire while SSE is active.
                // Defer health/state refreshes here: the active SSE stream is the
                // liveness signal, and the next non-streaming foreground pass will
                // run the normal health check instead of racing the stream.
                return
            }
