...
mod tests {
    ...
    fn remote_eval_target_parses_user_host_and_path() {
        let target: RemoteEvalTarget = "clawdio@example.com:/srv/fawx".parse().expect("target");

        assert_eq!(target.ssh_user, "clawdio");
    ...
    fn ssh_command_format_builds_expected_args() {
        let spec = ssh_command_spec("clawdio", "100.93.251.101", "cd '/srv/fawx' && cargo test");
    ...
    fn patch_application_builds_scp_and_git_apply_commands() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "ConnectTimeout=30",
                "/tmp/local.patch",
                "clawdio@100.93.251.101:/tmp/remote.patch",
            ]
        );
        assert_eq!(
            apply,
