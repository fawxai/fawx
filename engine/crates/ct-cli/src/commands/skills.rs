...
fn get_skills_dir() -> Result<PathBuf> {
    ...
    let skills_dir = home.join(".citros").join("skills");
...
fn load_manifest(manifest_path: &Path) -> Result<ct_skills::manifest::SkillManifest> {
    let manifest_content = fs::read_to_string(manifest_path)
        .with_context(|| format!("Failed to read manifest: {:?}", manifest_path))?;

    let manifest: ct_skills::manifest::SkillManifest =
        toml::from_str(&manifest_content).context("Failed to parse manifest")?;

    ct_skills::manifest::validate_manifest(&manifest).context("Manifest validation failed")?;
    Ok(manifest)
}

fn validate_manifest_fields(manifest: &ct_skills::manifest::SkillManifest) -> Result<()> {
...
fn validate_wasm(manifest: &ct_skills::manifest::SkillManifest, wasm_bytes: &[u8]) -> Result<()> {
    ...
    let loader = ct_skills::loader::SkillLoader::new(vec![]);
    loader
...
fn install_skill_files(
    manifest: &ct_skills::manifest::SkillManifest,
    wasm_path: &Path,
    manifest_path: &Path,
) -> Result<()> {
...
fn print_install_summary(manifest: &ct_skills::manifest::SkillManifest, skill_dir: &Path) {
    println!(
        "✓ Installed skill '{}' v{}",
...
fn print_empty_skills_message() {
    ...
    println!("  citros skill install <path-to-skill>");
}
...
fn read_manifest_for_listing(
    manifest_path: &Path,
) -> Result<ct_skills::manifest::SkillManifest, String> {
    let manifest_content = fs::read_to_string(manifest_path)
...
fn print_manifest_for_listing(manifest: &ct_skills::manifest::SkillManifest) {
...
fn format_capabilities(capabilities: &[ct_skills::manifest::Capability]) -> String {
    capabilities
        .iter()
