...
pub struct Options {
    ...
    pub listen: String,
    /// OAuth provider authorize endpoint URL. Falls back to `CITROS_OPENAI_AUTH_URL`.
    pub auth_url: Option<String>,
    /// OAuth provider token endpoint URL. Falls back to `CITROS_OPENAI_TOKEN_URL`.
    pub token_url: Option<String>,
    /// OAuth client ID. Falls back to `CITROS_OPENAI_CLIENT_ID`.
    pub client_id: Option<String>,
    /// OAuth client secret (optional). Falls back to `CITROS_OPENAI_CLIENT_SECRET`.
    pub client_secret: Option<String>,
    /// OAuth scope string. Falls back to `CITROS_OPENAI_SCOPE`, defaults to `openid profile email offline_access`.
...
impl BridgeSettings {
    fn from_options(options: &Options) -> anyhow::Result<Self> {
        let authorize_url = option_or_env(&options.auth_url, "CITROS_OPENAI_AUTH_URL")
            .context("Missing authorize URL. Set --auth-url or CITROS_OPENAI_AUTH_URL")?;
        let token_url = option_or_env(&options.token_url, "CITROS_OPENAI_TOKEN_URL")
            .context("Missing token URL. Set --token-url or CITROS_OPENAI_TOKEN_URL")?;
        let client_id = option_or_env(&options.client_id, "CITROS_OPENAI_CLIENT_ID")
            .context("Missing client ID. Set --client-id or CITROS_OPENAI_CLIENT_ID")?;
        let client_secret = option_or_env(&options.client_secret, "CITROS_OPENAI_CLIENT_SECRET");
        let scope = option_or_env(&options.scope, "CITROS_OPENAI_SCOPE")
...
fn validate_redirect_uri(uri: &str) -> Result<(), ApiError> {
    // Allow citros:// custom scheme and localhost for testing
    if uri.starts_with("citros://")
        ...
        || uri.starts_with("http://127.0.0.1")
    {
        ...
        Err(ApiError::bad_request(
            "Invalid redirect_uri: must use citros:// scheme or localhost",
        ))
    }
}
...
mod tests {
    ...
    async fn test_exchange_missing_state() {
        ...
        let session = LoginSession {
            ...
            redirect_uri: "citros://oauth/callback".to_string(),
    ...
    fn option_or_env_prefers_option_value() {
        ...
        let resolved = option_or_env(&option, "CITROS_TEST_MISSING");
    ...
    fn build_authorize_url_contains_required_parameters() {
        ...
        assert!(url.contains("redirect_uri=citros%3A%2F%2Foauth%2Fcallback"));
    ...
    #[test]
    fn redirect_uri_validation_allows_citros_scheme() {
        assert!(validate_redirect_uri("citros://oauth/callback").is_ok());
    }
