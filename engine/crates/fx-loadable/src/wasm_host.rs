...
use fx_skills::live_host_api::{
    execute_http_request, execute_http_request_envelope, CredentialProvider,
};
...
impl HostApi for LiveHostApi {
    ...
    fn http_request_v2(
        ...
        if !is_network_allowed(url, &self.capabilities) {
            tracing::error!("http_request_v2 denied: domain not in allowlist");
            return None;
        }
