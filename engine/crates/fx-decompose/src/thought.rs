...
impl ThoughtIdAllocator {
    ...
    fn advance_past(&mut self, id: ThoughtId) {
        let next_after_id = id.0.checked_add(1).expect("thought id overflow");
        self.next = self.next.max(next_after_id);
    }
}

impl Iterator for ThoughtIdAllocator {
    type Item = ThoughtId;
...
mod tests {
    ...
    fn fields_metadata(entries: [(&str, ThoughtMetadataValue); 3]) -> ThoughtMetadata {
    ...
    fn insert_returns_existing_id_and_advances_allocator() {
        ...
        let inserted_id = pool.insert(ThoughtState {
            id: ThoughtId(4),
            content: "manual seed".to_string(),
            score: Some(0.3),
            metadata: ThoughtMetadata::Empty,
            parent_ids: Vec::new(),
            origin_operation: Some(GraphNodeId(2)),
        });

        assert_eq!(inserted_id, ThoughtId(4));
        assert_eq!(
    ...
    fn create_allocates_sequential_ids_and_stores_state() {
        ...
        assert_eq!(root_id, ThoughtId(0));
        assert_eq!(child_id, ThoughtId(1));
    ...
    fn scored_returns_only_thoughts_with_scores() {
        ...
        let scored_ids: Vec<ThoughtId> = pool.scored().into_iter().map(|state| state.id).collect();
