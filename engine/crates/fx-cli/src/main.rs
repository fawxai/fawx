...
async fn run_tui() -> anyhow::Result<i32> {
    ...
    let (loop_engine, memory) = tui::build_loop_engine_and_memory_from_config(&config);
    let mut app = tui::TuiApp::new_with_memory(auth_manager, router, loop_engine, config, memory)?;
    app.run().await?;
    Ok(0)
}
