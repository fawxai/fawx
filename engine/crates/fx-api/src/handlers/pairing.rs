...
mod phase4_tests {
    ...
    fn qr_pairing_prefers_tailscale_hostname_when_cert_ready() {
        let response = qr_pairing_response(
            &test_runtime(false),
            &QrTailscaleStatus {
                hostname: Some("myhost.tail0000.ts.net".to_string()),
                cert_ready: true,
            },
        );
        assert_eq!(response.display_host, "myhost.tail0000.ts.net");
        assert_eq!(response.transport, "tailscale_https");
        assert!(!response.same_network_only);
        assert!(response.scheme_url.contains("host=myhost.tail0000.ts.net"));
    }
