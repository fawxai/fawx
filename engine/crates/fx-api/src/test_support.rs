...
pub(crate) struct StubAppEngine {
    ...
    dynamic_active_model: Option<String>,
...
impl StubAppEngine {
    ...
    pub(crate) fn with_dynamic_active_model(
        mut self,
        dynamic_active_model: impl Into<String>,
    ) -> Self {
        self.dynamic_active_model = Some(dynamic_active_model.into());
        self
    }
...
impl AppEngine for StubAppEngine {
    ...
    async fn available_models_dynamic(&mut self) -> Vec<ModelInfoDto> {
        if let Some(dynamic_active_model) = self.dynamic_active_model.clone() {
            self.active_model = dynamic_active_model;
        }
        self.dynamic_models
            .clone()
