...
- **No mid-stream message injection.** Steer/queue is client-side only in V1. See Steer chip section for V2 evolution.
...
- On `done` event: streaming indicator removed, message finalized with full response from `done.response`. If steer is queued, auto-send it.
...
| SSE streams | 300s (5 min) | 0 (unlimited) | Long-running agent turns with tool calls can take minutes. Setting resource timeout to 0 means no limit — the stream lives until `done`/`error` or client cancels. |
| Health check | 5s | 10s | Fast-fail for connection status |

**SSE keep-alive:** The server does not currently send SSE heartbeat/ping frames during long tool executions. If the model is running a tool for 2+ minutes with no tokens, the client may think the connection is dead. V1 mitigation: use the 300s request timeout and treat silence as "thinking." V2: add server-side SSE ping frames every 15s during tool execution.
...
- **On session select:** Always re-fetch full history from `GET /v1/sessions/{id}/messages`. Do not rely solely on in-memory cache.
- **On app foreground (iOS) / window focus (macOS):** Re-fetch the active session's history.
- **During streaming:** Do not re-fetch. Trust the local SSE-assembled message.
- **After stream completes:** The `done` event contains the full response. Record it locally. The server also persists it, so next re-fetch will include it.

This ensures the app stays consistent with server state even when compaction drops messages or another client modifies the session.
