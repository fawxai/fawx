...
enum SidebarSelection: Hashable, RawRepresentable {
    ...
    init?(rawValue: String) {
        if rawValue == "__skills__" { self = .skills }
        else { self = .session(rawValue) }
    }

    var rawValue: String {
        switch self {
        case .skills: return "__skills__"
        case .session(let key): return key
        }
    }
}
// Settings is NOT in this enum — it opens a separate Settings window via ⌘,
// Sentinel value "__skills__" cannot collide with session keys (UUIDs/hex strings).
```
...
3. **On return to `.active`:**
   ...
   - **If a queued steer message exists:** Do NOT auto-send. Show the queued chip and let the user decide.
...
- **Canonical format for Keychain keying:** The stored URL string is normalized before storage AND before Keychain lookup using this algorithm:
  ...
  func canonicalizeServerURL(_ input: String) -> String? {
      ...
      // Reject if host is still nil after normalization (e.g., empty input)
      guard components.host != nil, !components.host!.isEmpty else { return nil }
