# Citros UI Spec — The Floating Bubble

*Horizon 1: Android PoC Primary UI Surface*

> **Purpose:** This spec defines the visual design, interaction model,
> and animation behavior for Citros's floating bubble — the always-present
...
Citros's icon is a **PLACEHOLDER_SUPERCITROS** — a celestial orb that transforms
between states. During onboarding, the user selects a color from a
curated palette. This color becomes their personal Citros colorway,
applied across all UI elements (bubble, overlays, text, confirmation
dialogs).

Reference image: `docs/assets/PLACEHOLDER_SUPERCITROS-reference.jpg`
...
- **Ambient, not intrusive** — Citros should feel like a presence, not
  a notification
- **State is always visible** — the bubble's appearance tells the user
  what Citros is doing without requiring interaction
- **Personal** — the user's chosen color and personality make Citros
  feel like *theirs*
...
- Simple tap triggers haptic feedback (acknowledgment) but does NOT
  wake Citros — prevents accidental activation
...
Citros is executing actions on the phone.
...
**Haptic pattern:**
- Subtle, rhythmic pulses while Citros works
...
| Element | Behavior |
|---------|----------|
| Citros orb | Hovers over / near the tap target |
...
- Useful for learning what Citros can do
...
- Citros should acknowledge the interrupt (visual + haptic feedback)
...
### Topic 6: Horizon 2 — CitrosOS Modes
