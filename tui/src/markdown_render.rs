...
fn supports_code_block_coloring(lang: &str) -> bool {
    matches!(
        lang,
        "bash"
            | "c"
    ...
    I: Iterator<Item = Event<'a>>,
{
    ...
    fn start_codeblock(&mut self, lang: Option<String>, indent: Option<Span<'static>>) {
        ...
        let lang = lang
            ...
            .filter(|s| supports_code_block_coloring(s));
    ...
    fn end_codeblock(&mut self) {
        let has_known_lang = self.code_block_lang.take().is_some();
        let code = std::mem::take(&mut self.code_block_buffer);
        if !code.is_empty() {
            for line in code.lines() {
                let span = if has_known_lang {
                    Span::styled(line.to_string(), self.styles.code)
                } else {
...
mod tests {
    ...
    fn fenced_code_info_string_with_metadata_highlights() {
        ...
        for info in &["rust,no_run", "rust no_run", "rust title=\"demo\""] {
            ...
            assert!(
                has_color,
                "info string \"{info}\" should still preserve code-block coloring"
            );
        }
    }
